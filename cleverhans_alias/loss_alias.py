...
class CrossEntropy(Loss):
  ...
  def fprop(self, x, y, **kwargs):
    print("DEBUG::prior kwargs.update(self.kwargs)")
    kwargs.update(self.kwargs)
    print("DEBUG::prior loop)")
    if self.attack is not None:
      print("DEBUG::prior attack_params = copy.copy(self.attack_params))")
      attack_params = copy.copy(self.attack_params)
      if attack_params is None:
        attack_params = {}
      if self.pass_y:
        attack_params['y'] = y
      print("DEBUG::prior x = x, self.attack.generate(x, **attack_params)")
      x = x, self.attack.generate(x, **attack_params)
      coeffs = [1. - self.adv_coeff, self.adv_coeff]
      print("DEBUG::prior if self.adv_coeff == 1.:")
      if self.adv_coeff == 1.:
        x = (x[1],)
        coeffs = (coeffs[1],)
    else:
      print("DEBUG::prior x = tuple([x])")
      x = tuple([x])
      coeffs = [1.]
    assert np.allclose(sum(coeffs), 1.)

    # Catching RuntimeError: Variable -= value not supported by tf.eager.
    print("DEBUG::prior try")
    try:
      print("DEBUG::prior y -= self.smoothing * (y - 1. / tf.cast(y.shape[-1], y.dtype))")
      y -= self.smoothing * (y - 1. / tf.cast(y.shape[-1], y.dtype))
    except RuntimeError:
      print("DEBUG::prior y.assign_sub(self.smoothing * (y - 1. / tf.cast(y.shape[-1], y.dtype)))")
      y.assign_sub(self.smoothing * (y - 1. / tf.cast(y.shape[-1],
                                                      y.dtype)))

    print("DEBUG::prior logits = [self.model.get_logits(x, **kwargs) for x in x]")
    logits = [self.model.get_logits(x, **kwargs) for x in x]
    print("DEBUG::prior loss = sum(...")
    loss = sum(
        coeff * tf.reduce_mean(softmax_cross_entropy_with_logits(labels=y,
                                                                 logits=logit))
        for coeff, logit in safe_zip(coeffs, logits))
    print("DEBUG::about to exit the fprop function")
    return loss
