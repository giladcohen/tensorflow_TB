'''
Same as test3.py, except using WRN-28-10 with max pool and
an additional fully connected layer.
Using ensemble of 9 WRN-28-10, without "pool_in".
'''
...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('mode', 'global_democracy', 'Mode of estimating the true label')
...
if (FLAGS.K != -1):
    ...
    k_arr = np.array([1,3,5,7,9,11,31,51,101,151,201,301,401,501,1000,5000,2000,3000])
...
def err_report(label_est, true_labels, label_vote=None):
    ...
    for ind in range(N_test):
        if (label_vote!=None and len(np.nonzero(label_vote[ind]==max(label_vote[ind]))[0])>1):
...
if (FLAGS.mode == 'majority_vote'):
    ...
    for k in np.nditer(k_arr):
        ...
        if (FLAGS.mode == 'fc1_vote'):
            ...
            hist = np.apply_along_axis(lambda x: np.bincount(x, minlength=10), axis=1, arr=neighbors_all)
            hist_first_choices = np.apply_along_axis(lambda x: np.bincount(x, minlength=N_logs+1), axis=1, arr=first_choice)
