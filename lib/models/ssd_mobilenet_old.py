...
class SSDMobileNet(ModelBase):
    ...
    def _build_loss(self):
        ...
        #     self.num_classes)
        # self.input_tensor = tf.cond(self.is_training, lambda: self.train_input_tensor, lambda: self.test_input_tensor)
        input_tensor = self.train_input_tensor
        # batch_size = tf.cond(self.is_training, lambda: self.prm.train.train_control.TRAIN_BATCH_SIZE, lambda: self.prm.train.train_control.EVAL_BATCH_SIZE)
        batch_size = self.prm.train.train_control.TRAIN_BATCH_SIZE

        images                    = input_tensor[0]
        images.set_shape([batch_size, None, None, 3])
        images_list               = tf.unstack(images)

        groundtruth_boxes         = input_tensor[1]
        groundtruth_boxes.set_shape([batch_size, None, 4])
        groundtruth_boxes_list    = tf.unstack(groundtruth_boxes)

        groundtruth_classes       = input_tensor[2]
        groundtruth_classes.set_shape([batch_size, None, 4])  # 4 classes
        groundtruth_classes_list  = tf.unstack(groundtruth_classes)

        groundtruth_masks_list     = None
        groundtruth_keypoints_list = None

        resized_images, true_images_shape = self.ssd_meta_arch.preprocess(images)
        ...
        losses_dict     = self.ssd_meta_arch.loss(prediction_dict, true_images_shape)
