...
class ResNet(NoRefModel):
  ...
  def build(self, layers, input_shape):
      ...
      if isinstance(layers[-1], Softmax):
        ...
        layers[-3].name = 'embeddings'
      else:
        layers[-1].name = 'logits'
        layers[-2].name = 'embeddings'
...
class Conv2D(Layer):
  ...
  def __init__(self, num_residual_units=5):
...
def make_wresnet(nb_classes=10, input_shape=(None, 32, 32, 3), scope=None):
  layers = [Input(),
            Conv2D(num_residual_units=4),  # the whole ResNet is basically created in this layer
            Flatten(),
            Linear(nb_classes),
            Softmax()]
