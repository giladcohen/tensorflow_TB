...
steps, acc_values = load_data_from_csv_wrapper(csv_file, mult=1.0, round_points=8)
P_SAME_mnist_lenet = [ma * acc + md * (1.0 - acc) for ma, md, acc in zip(ma_values, md_values, acc_values)]
P_SAME_mnist_lenet = [round(elem, 4) for elem in P_SAME_mnist_lenet]
...
steps, acc_values = load_data_from_csv_wrapper(csv_file, mult=1.0, round_points=8)
P_SAME_mnist_fc2net = [ma * acc + md * (1.0 - acc) for ma, md, acc in zip(ma_values, md_values, acc_values)]
P_SAME_mnist_fc2net = [round(elem, 4) for elem in P_SAME_mnist_fc2net]

ax1 = fig.add_subplot(131)
ax1.plot(steps, P_SAME_mnist_wrn   , 'r')
ax1.plot(steps, P_SAME_mnist_lenet , 'g')
ax1.plot(steps, P_SAME_mnist_fc2net, 'b')

ax1.set_ylabel('$P_{SAME}$', labelpad=5, fontdict={'fontsize': 12})
ax1.set_ylim(bottom=0.0, top=1.045)
ax1.yaxis.grid()
ax1.set_title('MNIST')
ax1.legend(['Wide Resnet', 'LeNet', 'MLP-640'], loc=(0.4, 0.06), prop={'size': 12})
...
ax2.legend(['Wide Resnet', 'LeNet', 'MLP-640'], loc=(0.4, 0.06), prop={'size': 12})
...
ax3.legend(['Wide Resnet', 'LeNet', 'MLP-640'], loc=(0.4, 0.06), prop={'size': 12})
