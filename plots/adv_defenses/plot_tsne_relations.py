...
# for vis_idx in range(len(X_val)):
    ...
    # plotting the TSNE:
    plt.figure(2)
    plt.scatter(neighbors_tsne_embeddings[:, 0]    , neighbors_tsne_embeddings[:, 1]    , s=5, marker='o', c='blue', alpha=0.5)
    plt.scatter(neighbors_tsne_embeddings_adv[:, 0], neighbors_tsne_embeddings_adv[:, 1], s=5, marker='o', c='red' , alpha=0.5)
    # TODO(gilad): plot overlapping neighbors in a merged color

    plt.scatter(top_helpful_tsne_embeddings[:, 0]    , top_helpful_tsne_embeddings[:, 1]    , s=5, marker='s', c='blue')
    plt.scatter(top_helpful_tsne_embeddings_adv[:, 0], top_helpful_tsne_embeddings_adv[:, 1], s=5, marker='^', c='red')
